...
const testConfig = require('../lib/calculator.config')
const dateDiffInHours = (date1, date2) => (date1.valueOf() - date2.valueOf()) / 60 / 60 / 1000
const getNightTimeInHours = () => {
  const start = new Date(0).setUTCHours(testConfig.WORKDAYEND)
  const end = new Date(0).setUTCHours(testConfig.WORKDAYEND + (24 - testConfig.WORKDAYLENGTH))
  const hours = dateDiffInHours(end, start)
  return hours
}

const setupTurnaround = (days, overFlow = 0) => {
  return (testConfig.WORKDAYLENGTH * days) + overFlow
}
...
test('should be able to handle more than two workdays hours turnaround time', () => {
  ...
  const turnaround = setupTurnaround(2, (testConfig.WORKDAYEND - ticketCreatedAt.getUTCHours()))
  const dueTime = calculator.getDueTime(ticketCreatedAt, turnaround)
  const diffHours = dateDiffInHours(dueTime, ticketCreatedAt)
  expect(diffHours).toBe(turnaround + (nightTimeInHours * (parseInt(turnaround / testConfig.WORKDAYLENGTH) + 1)))
})

test('should handle start times with more than one hour remaining on workday', () => {
  const ticketCreatedAt = new Date('2018-08-22T09:12Z')
  const overFlow = 1
  const turnaround = setupTurnaround(2, overFlow)
  const dueTime = calculator.getDueTime(ticketCreatedAt, turnaround)
  const diffHours = dateDiffInHours(dueTime, ticketCreatedAt)
  expect(diffHours).toBe(turnaround + (nightTimeInHours * parseInt(turnaround / testConfig.WORKDAYLENGTH)))
})
...
test('should skip two weekends', () => {
  ...
  const overFlow = 2
  const days = 8
  const turnaround = setupTurnaround(days, overFlow)
  const dueTime = calculator.getDueTime(ticketCreatedAt, turnaround)
  const diffHours = dateDiffInHours(dueTime, ticketCreatedAt)
...
test('should skip more than two weekends', () => {
  const ticketCreatedAt = new Date('2018-08-22T09:12Z')
  const days = 13
  const turnaround = setupTurnaround(days, 1)
  const dueTime = calculator.getDueTime(ticketCreatedAt, turnaround)
  const diffHours = dateDiffInHours(dueTime, ticketCreatedAt)
  expect(diffHours).toBe(turnaround + (weekendHours * 3) + nightTimeInHours * days)
})

test('should skip more than two weekends with friday ending', () => {
  const ticketCreatedAt = new Date('2018-08-22T09:12Z')
  const days = 12
  const turnaround = setupTurnaround(days)
  const dueTime = calculator.getDueTime(ticketCreatedAt, turnaround)
  const diffHours = dateDiffInHours(dueTime, ticketCreatedAt)
